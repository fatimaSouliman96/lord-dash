...
export default function EditRegion({
    ...
    fetchData: () => void;
    id: React.ReactNode;
    oldName: React.ReactNode;
    cityId: React.ReactNode;
    cities: city[]
}) {

    const [name, setName] = useState<string>(oldName ? String(oldName) : "")
    const [idCity, setIdCity] = useState<number>(cityId?Number(cityId):0)
    console.log(cityId)
